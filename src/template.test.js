...
const expectTemplateOutputToMatch = function (expectedResult, result) {
...
describe("template tag function", () => {
  ...
  test("renders template with object value", () => {
    const value = {
      foo: "bar",
    };
    const result = template`
      ${value}
    `;
    expect(result).toMatch(/{\"foo\":\"bar\"}/i);
  });
});
...
describe("processes template", () => {
  test("with single element", () => {
    ...
    const expectedResult = "<div>foo</div>";
