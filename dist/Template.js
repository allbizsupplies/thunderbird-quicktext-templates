...
return {
  subject: () => {
    const [orderID] = getInputs([{ label: "Order ID" }]);
    return `${orderID ? `[Order #${orderID}]` : ""} Your artwork needs to be fixed`;
  },
  body: () => {
    const orderID = parseOrderIDFromSubject();
    const [projectName, artworkVersion] = getInputs([
      { label: "Project name" },
      {
...
return {
  subject: () => {
    const [orderID] = getInputs([{ label: "Order ID" }]);
    return `${orderID ? `[Order #${orderID}]` : ""} Hardcopy proof is ready to view`;
  },
  body: () => {
    const orderID = parseOrderIDFromSubject();
    const [projectName, servicePriority] = getInputs([
...
return {
  subject: () => {
    const [orderID] = getInputs([{ label: "Order ID" }]);
    return `${orderID ? `[Order #${orderID}]` : ""} Proof approval required`;
  },
  body: () => {
    const orderID = parseOrderIDFromSubject();
    const [projectName, servicePriority, artworkCharge] = getInputs([
      { label: "Project name" },
      {
        label: "Service priority",
        options: servicePriorityOptions,
